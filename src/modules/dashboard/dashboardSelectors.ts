...
export function thisWeeksScheduleBlocks(tasks: Task[], today: Date): Task[] {
  ...
  // format(), not toISOString(): occurrenceDate is a plain yyyy-MM-dd with no
  // timezone, but toISOString() converts through UTC, which shifts the
  // boundary by a day in any zone not exactly UTC+0 — format() reads the local
  // wall-clock date instead, matching how taskRecurrence.ts computes the same
  // boundary.
...
// --- Panel 3: prep scorecard vs. monthly targets ------------------------------
//
// Deliberately not reimplemented here: use prepScorecard.ts's scorecardFor and
// weakestTopics directly. This module only aggregates ACROSS modules; the
// within-module scorecard math already exists and is already tested there.
//
// The "vs. targets" half of this panel needs engineering_first_roadmap_v2.md's
// Section 15 numbers, which are not in this repo. scorecardFor computes actuals
// only — comparing them to a target is blocked until that file exists.
...
export function findGateChecklistTask(
  tasks: Task[],
  date: Date,
): Task | null {
